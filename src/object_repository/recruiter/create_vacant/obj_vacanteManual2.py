...
from src.services.peticiones_HTTP import send_patch
...
def job_description(vacant_id, headers):
    try:
        ...
        resultado = send_patch(url, headers, my_body, 200)
        if resultado != 0:
            print('Se hizo la descripción del trabajo :) \n')
            return 'Se hizo la descripción del trabajo', 1
        else:
            print('No se hizo la descripción del trabajo')
            return 'No se hizo la descripción del trabajo', 0
    except Exception as e:
        print('No se hizo el segundo paso de la creación de la vacante :( \n', e)
        return 'No se hizo el segundo paso de la creación de la vacante', 0
